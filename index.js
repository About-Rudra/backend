...
// COMPANY DETAILS EDIT POST REQUEST
    ...
    // get company posted internship
app.get("/getinternshipinfo", async (req, res) => {
  try {
...
// get posted students new internship

app.get("/getappliedinternshipinfo", async (req, res) => {
  try {
      // Retrieve specific internship post information from the database
      const sudentInfo = await db.query(
          "SELECT * FROM internship_application;"
      );

      console.log("Applied Students Information:", sudentInfo.rows);

      res.status(200).json(sudentInfo.rows);
  } catch (error) {
